...
✅ **Chapter 4 | Lesson 11 of 11** — Chapter 4 এর শেষ এবং সবচেয়ে Advanced Lesson!
...
এগুলো সব DevOps/SysAdmin level এর real-world networking concept। চলো শুরু করি!
...
ধরো তোমার কাছে দুটো Network Card (NIC) আছে — `eth0` আর `eth1`। তুমি চাও:
- যদি একটা NIC নষ্ট হয়ে যায়, তাহলেও network চালু থাকুক **(Failover)**
- অথবা দুটো NIC মিলিয়ে Speed দ্বিগুণ করতে চাও **(Load Balancing)**

**Bonding** মানে হলো এই দুটো NIC কে একটা **virtual interface** এ বেঁধে ফেলা — নাম হয় `bond0`।
...
**Step 1: Config file দেখো**
```bash
ls /etc/netplan/
# Output: 00-installer-config.yaml বা 01-netcfg.yaml
```

**Step 2: Bonding config লেখো**
```bash
...
network:
  ...
  ethernets:
    eth0:
      dhcp4: false    # eth0 কে slave হিসেবে দেবো
    eth1:
      dhcp4: false    # eth1 কেও slave হিসেবে দেবো
  bonds:
    bond0:
      interfaces: [eth0, eth1]   # এই দুটো NIC bond করো
...
```bash
# Step 1: bond0 interface তৈরি করো
sudo nmcli con add type bond \
    con-name bond0 \
    ifname bond0 \
    bond.options "mode=active-backup,miimon=100"

# Step 2: eth0 কে slave হিসেবে যোগ করো
sudo nmcli con add type ethernet \
    con-name bond0-slave-eth0 \
    ifname eth0 \
    master bond0

# Step 3: eth1 কেও slave হিসেবে যোগ করো
sudo nmcli con add type ethernet \
    con-name bond0-slave-eth1 \
    ifname eth1 \
    master bond0

# Step 4: IP assign করো
sudo nmcli con mod bond0 \
    ipv4.addresses "192.168.1.10/24" \
    ipv4.gateway "192.168.1.1" \
    ipv4.method manual

# Step 5: Connection চালু করো
...
network:
  ...
  bridges:
    br0:
      interfaces: [eth0]    # eth0 কে bridge এ যোগ করো
...
# eth0 কে bridge এ যোগ করো (slave করো)
...
# Bridge এর MAC address table দেখো (কোন MAC কোন port এ আছে)
brctl showmacs br0
...
## 🏷️ PART 3: VLANs (Virtual Local Area Networks)

### VLAN কী?

ধরো একটা office এ ৩টা department — HR, Finance, Engineering। সবাই একই physical network switch এ connected। কিন্তু তুমি চাও HR এর traffic Finance দেখতে না পাক।

এটা করার দুটো উপায়:
1. ৩টা আলাদা physical switch কেনো — **expensive!**
2. **VLAN** ব্যবহার করো — একটাই switch, কিন্তু logically আলাদা নেটওয়ার্ক — **smart!**
...
**প্রথমে `8021q` kernel module load করো:**

```bash
sudo modprobe 8021q

# Check করো module load হয়েছে কিনা
lsmod | grep 8021q
# Output:
# 8021q                  32768  0
# garp                   16384  1 8021q
...
```bash
# eth0 এর উপর VLAN 10 তৈরি করো
sudo ip link add link eth0 name eth0.10 type vlan id 10

# eth0 এর উপর VLAN 20 তৈরি করো
sudo ip link add link eth0 name eth0.20 type vlan id 20

# Interface গুলো চালু করো
...
```bash
# === BONDING ===
cat /proc/net/bonding/bond0          # Bond status দেখো
ip addr show bond0                   # Bond IP দেখো
ip link show bond0                   # Bond link status

# === BRIDGING ===
brctl show                           # সব bridge দেখো
bridge link show                     # Bridge ports দেখো
bridge fdb show br0                  # MAC table দেখো
ip addr show br0                     # Bridge IP দেখো

# === VLAN ===
cat /proc/net/vlan/                  # সব VLAN দেখো
ip -d link show eth0.10              # VLAN details দেখো
...
- 🏷️ **VLAN** — একটা Physical network কে logically আলাদা করা হয় VLAN tag দিয়ে। প্রতিটা VLAN আলাদা subnet — traffic isolate থাকে।
- ⚙️ **`8021q` module** — VLAN support এর জন্য অবশ্যই load করতে হবে।
- 📄 **netplan/nmcli** — permanent config এর জন্য এগুলো ব্যবহার করো, `ip` command শুধু temporary।

---

## 💪 Practice Tasks

1. `/proc/net/bonding/` folder টা দেখো — যদি কোনো bonding না থাকে তাহলে কী দেখায়?
   ```bash
   ls /proc/net/bonding/
   cat /proc/net/vlan/   # VLAN ও দেখো
   ```

2. `8021q` module load করো এবং verify করো:
   ```bash
   sudo modprobe 8021q
   lsmod | grep 8021q
   ```

3. তোমার system এর network interface দেখো এবং কোনটা দিয়ে VLAN তৈরি করা যাবে সেটা চিহ্নিত করো:
   ```bash
   ip link show
   ip addr show
   ```
...
তুমি **Chapter 4: Networking in Linux** এর সব ১১টা Lesson শেষ করেছো! 🏆

### Chapter 4 এ যা শিখেছো:
...
Shell Scripting শিখলে তুমি নিজেই automation tool বানাতে পারবে — এটা DevOps এর সবচেয়ে exciting part! 🚀

যখন ready হবে বলো **"Chapter 5, Lesson 1"** — শুরু করি! 💪