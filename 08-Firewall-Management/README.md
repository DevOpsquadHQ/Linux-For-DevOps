...
কল্পনা করো তোমার বাড়িতে একটা **security guard** আছে। সে সিদ্ধান্ত নেয় — কোন visitor ভেতরে যাবে, কোনটা যাবে না। Linux-এ **Firewall** ঠিক এই কাজটাই করে।

> **Firewall** = একটা software-based security layer যে decide করে কোন network traffic তোমার system-এ **ঢুকতে পারবে**, **বের হতে পারবে**, বা **forward হবে**।
...
```
Network Packet আসলে:
Internet → [Netfilter/iptables rules] → তোমার System
```
...
│              filter table                   │
│                                             │
│  INPUT   → তোমার system-এ আসা traffic      │
│  OUTPUT  → তোমার system থেকে যাওয়া traffic │
│  FORWARD → তোমার system দিয়ে pass করা      │
└─────────────────────────────────────────────┘
```
...
- **FORWARD** = তোমার বাড়ির সামনে দিয়ে কেউ যাচ্ছে (তুমি router)
...
```bash
# Port 80 (HTTP) allow করো
sudo iptables -A INPUT -p tcp --dport 80 -j ACCEPT

# Port 443 (HTTPS) allow করো
sudo iptables -A INPUT -p tcp --dport 443 -j ACCEPT

# Port 22 (SSH) allow করো
...
```bash
# Port 23 (Telnet) DROP করো — কোনো response দেবে না
...
এটা অনেক গুরুত্বপূর্ণ! তুমি যদি সব INPUT block করো, তাহলে তোমার নিজের করা request-এর response-ও আসবে না। তাই:

```bash
# Already established connection-এর traffic allow করো
sudo iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
```

**Analogy:** তুমি Google-এ request করলে, Google-এর reply যেন আসতে পারে — এই rule সেটা নিশ্চিত করে।
...
```bash
# সব incoming traffic block করো (default)
sudo iptables -P INPUT DROP

# সব outgoing traffic allow করো
...
`ufw` আসলে `iptables`-এরই একটা **wrapper** — ভেতরে iptables চলে, কিন্তু তুমি সহজ commands ব্যবহার করো।
...
# Rule নম্বর দিয়ে delete করো
sudo ufw delete 2

# অথবা directly rule উল্লেখ করে
...
```bash
# Specific IP থেকে SSH allow করো
...
# Default zone change করো
sudo firewall-cmd --set-default-zone=public

# Interface কোন zone-এ আছে
sudo firewall-cmd --get-zone-of-interface=eth0

# Interface কে zone-এ assign করো
...
- সবসময় SSH allow করার **পরে** default DROP set করো
...
**Task 1 (ufw):**
```
Ubuntu system-এ ufw দিয়ে এই setup করো:
- Default: সব incoming block
- SSH allow করো
- HTTP ও HTTPS allow করো
- তোমার নিজের IP থেকে port 3000 allow করো
- Status verify করো
```
...
- Port 22 allow করো
- Port 80 allow করো
- একটা নির্দিষ্ট IP (যেকোনো একটা নাও) block করো
- সব rules list করো line number সহ
- একটা rule delete করো
```

**Task 3 (Troubleshooting):**
```
তুমি একটা web server setup করেছো port 8080-এ,
...
পরের lesson-এ শিখবো কীভাবে network-এর **actual packets** দেখতে হয় — real-time-এ কী data যাচ্ছে-আসছে তা capture ও analyze করা। DevOps এবং security debugging-এর জন্য অনেক কাজের skill! 🔍