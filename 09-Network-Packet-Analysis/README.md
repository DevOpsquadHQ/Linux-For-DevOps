...
আজকে আমরা শিখবো **tcpdump** — Linux-এর সবচেয়ে শক্তিশালী network packet analysis tool। এটা দিয়ে তুমি literally network-এর ভেতর দিয়ে যাওয়া সব data "দেখতে" পারবে।

---

## 🧠 tcpdump কী? — Analogy দিয়ে বোঝো

কল্পনা করো তুমি একটা **ব্যস্ত highway-এর পাশে দাঁড়িয়ে আছো** এবং প্রতিটা গাড়ির number plate, কোথা থেকে আসছে, কোথায় যাচ্ছে — সব নোট করছো।

**tcpdump** ঠিক এই কাজটাই করে — শুধু highway-এর বদলে এটা তোমার **network interface** monitor করে এবং প্রতিটা **packet** (data packet) কোথা থেকে আসছে, কোথায় যাচ্ছে সব দেখায়।
...
> ⚠️ **Important:** tcpdump চালাতে **root/sudo** লাগে কারণ এটা network interface directly access করে।
...
## 🚀 Command 1: সবচেয়ে Simple — সব packets দেখো

```bash
sudo tcpdump
```

**কী করে:** তোমার default network interface-এ আসা-যাওয়া সব packet দেখায়।
...
> ⚠️ Ctrl+C দিয়ে বন্ধ করো — নইলে সব packet দেখাতেই থাকবে!

---

## 🖥️ Command 2: নির্দিষ্ট Interface দেখো (`-i`)

```bash
# কোন কোন interface আছে দেখো
sudo tcpdump -D
...
**DevOps Use Case:** তোমার server-এ multiple network card থাকলে (public + private network) নির্দিষ্টটা monitor করতে পারবে।
...
### পরে পড়ো:
```bash
# Save করা file থেকে পড়ো
sudo tcpdump -r /tmp/capture.pcap
```

**এটা কেন দরকার DevOps-এ?**
```
✅ Production issue investigate করতে capture করে রাখো
✅ Wireshark (GUI tool)-এ open করে visually analyze করো
✅ Team-এর সাথে share করো
✅ Later analysis-এর জন্য archive করো
```

---

## 🔎 Command 5: Filter করো — এটাই tcpdump-এর আসল শক্তি!

### 5.1 — নির্দিষ্ট Host-এর traffic দেখো
...
### 5.2 — নির্দিষ্ট Port-এর traffic দেখো
...
**DevOps Use Case:**
```bash
# App কি সত্যিই database connect করছে? দেখো:
...
**Example — ping করলে দেখো কী হয়:**
```bash
# Terminal 1-এ চালাও:
sudo tcpdump -i eth0 icmp

# Terminal 2-এ ping করো:
ping -c 3 8.8.8.8
```
...
## 👁️ Command 6: Verbose Output — বিস্তারিত দেখো
...
## 🔤 Command 7: Packet-এর Content দেখো (`-A` এবং `-X`)

```bash
# ASCII format-এ packet content দেখো (readable text)
sudo tcpdump -i eth0 -A port 80 -c 5

# Hex + ASCII উভয়ই দেখো
...
### Scenario 4: SSH Brute Force Attack detect করো
```bash
# Port 22-তে অনেক বেশি connection attempt দেখো
sudo tcpdump -nn -i eth0 tcp dst port 22 and 'tcp[tcpflags] == tcp-syn'
```

### Scenario 5: Kubernetes Pod-to-Pod Communication Debug
```bash
# Kubernetes node-এ চালাও, pod network interface monitor করো
...
## ⚡ Most Useful One-Liners (মুখস্থ রাখো!)

```bash
# 1. সব HTTP traffic দেখো
sudo tcpdump -nn -i any port 80 -A

# 2. সব DNS query দেখো
sudo tcpdump -nn -i any udp port 53

# 3. নির্দিষ্ট host-এর সব traffic capture করো file-এ
...
- ✅ **`-w`** দিয়ে save করো, **`-r`** দিয়ে পরে পড়ো
- ✅ **`-nn`** সবসময় ব্যবহার করো — faster ও cleaner
...
**Task 1:**
```
নিজের machine-এ tcpdump চালাও এবং শুধু ICMP traffic capture করো।
তারপর আরেকটা terminal থেকে `ping 8.8.8.8` করো এবং দেখো tcpdump কী দেখায়।
```

**Task 2:**
```
DNS traffic capture করো (udp port 53)।
তারপর `nslookup google.com` চালাও এবং দেখো DNS query ও response
tcpdump-এ কীভাবে দেখা যাচ্ছে।
```

**Task 3:**
```
100টা packet capture করে একটা file-এ save করো।
তারপর সেই file থেকে পড়ো এবং শুধু TCP packets filter করো।
```
...
> এই lesson-এ আমরা শিখবো Linux-এর সবচেয়ে গুরুত্বপূর্ণ network configuration files — কোথায় hostname mapping হয়, DNS কীভাবে configure হয়, এবং network interface কীভাবে সেটআপ করতে হয়। DevOps-এ এই files না জানলে অনেক কিছুই অন্ধকারে থাকে! 🌐